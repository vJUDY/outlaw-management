...
/** Get all users with plain password (admin only) */
function listUsers(includePwd = false) {
  const result = [];
  for (const [name, { rank, plain }] of users) {
    result.push({ name, rank, ...(includePwd ? { password: plain || '1234' } : {}) });
  }
  return result;
}
...
/** Sync users from sheet data (add new, remove deleted, keep existing passwords) */
async function syncFromSheet(staffArray) {
  const sheetNames = new Set(staffArray.map(s => s.name));

  // Remove users no longer in sheet
  ...
  // Add new or update rank of existing
